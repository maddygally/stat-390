...
double x1 = 2.0 // X coordinate (in pixels)
double y1 = 20.0 // Y coordinate (in pixels)
double width1 = 300.0 // Width of the annotation (in pixels)
double height1 = 200.0 // Height of the annotation (in pixels)

// Create a rectangle ROI based on the specified parameters
def roi1 = new qupath.lib.roi.RectangleROI(x1, y1, width1, height1)

// Create a new annotation object
def annotation = new qupath.lib.objects.PathAnnotationObject(roi1)

// Set the name and other properties if desired
annotation.setName("New Annotation1")
...
// Define the output directory (make sure to adjust the path as needed)
def outputDir = new File(homeDir, "Desktop/Qupath1")
...
// Export each annotation as a separate file
annotations.eachWithIndex { annot, index ->
    def roi = IJTools.convertToIJRoi(annot.getROI(), x_exp, y_exp, downsample)
    roiMan.addRoi(roi)

    // Define the filename for each ROI
    def filename = new File(outputDir, "roi_${index + 1}.roi") // You can customize the filename format change back to
    roiMan.runCommand("Save", filename.getAbsolutePath())
}

println("Annotations saved to: " + outputDir.getAbsolutePath())
